...
zoneight234
7pqrstsixteen`;
let lst2 = data2.split("\n");

let lst = data.split("\n");
const digits = [["one", "1"], ["two", "2"], ["three", "3"], ["four", "4"], ["five", "5"], ["six", "6"], ["seven", "7"], ["eight", "8"], ["nine", "9"]];

function first_and_last(string) {
    function is_digit_front(i, len) {
        for (let j = 0; j < digits.length; j++) {
            const digit = digits[j];
            const head = digit[0];
            const tail = digit[1];
            if (len - i >= head.length && string.slice(i, i + head.length) === head) {
                return tail;
            }
        }
        return NaN;
    }
    function is_digit_back(i, len) {
        for (let j = 0; j < digits.length; j++) {
            const digit = digits[j];
            const head = digit[0];
            const tail = digit[1];

            if (i >= head.length - 1 && string.slice(i - head.length + 1, i + 1) === head) {
                return tail;
            }
        }
        return NaN;
    ...
    for (let i = 0; i < len; i++) {
        ...
        let digit = is_digit_front(i, len);
        if (!isNaN(digit)) {
            first = digit;
            break;
        }
...
function add_all(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        const string = arr[i];
        sum = sum + first_and_last(string);
    }
    return sum;
}

console.log(add_all(lst));
