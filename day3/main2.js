const data = `416.........................559...............417...............785.......900.......284...........503...796....992..........................
...
for (let i = 0; i < symbols.length; i++) {
    ...
    filtered = filtered.replaceAll(symbol, ".");
}
filtered = filtered.replaceAll("*", "o");
filtered = filtered.split("\n");
// find all the positions of the stars
const star_pos = [];
for (let i = 0; i < filtered.length; i++) {
    star_pos[i] = [];
    let row = filtered[i];
    while (row.search("o") !== -1) {
        const temp = row.search("o");
        row = row.replace("o", ".");
        star_pos[i][star_pos[i].length] = temp;
    }
    filtered[i] = row;
}

console.table(filtered);
console.table(star_pos);


function find_adjacent(i, j) { // returns a list of coords adjacaent to the star
    return [[i-1,j-1],[i-1,j],[i-1,j+1],[i,j-1],[i,j+1],[i+1,j-1],[i+1,j],[i+1,j+1]];
}

function num_of_dots(n) {
    return n === 1
        ? "."
        : "." + num_of_dots(n - 1);
...
function find_number(i,j) { // given a coord, if not dot then check left until not number then check right until not number
    let row = filtered[i];
    if (row.charAt(j) === ".") {
        return 0;
    }
    let left = j;
    let right = j;
    while (row.charAt(left - 1) !== "." && left > 0) {
        left -= 1;
    }
    while (row.charAt(right) !== "." && right < row.length) {
        right += 1;
    }
    
    const number = row.slice(left, right);
    filtered[i] = row.slice(0, left) + num_of_dots(number.length) + row.slice(right, row.length);
...
for (let i = 0; i < star_pos.length; i++) {
    for (let j = 0; j < star_pos[i].length; j++) {
        let adjacent = find_adjacent(i,star_pos[i][j]); // gives a list of adjacent coords;
        let num_of_adjacent = 0;
        let product = 1;
        for (let k = 0; k < adjacent.length; k++) {
            let result = find_number(adjacent[k][0], adjacent[k][1]);
            if (result !== 0) {
                num_of_adjacent += 1;
                product *= result;
            }
